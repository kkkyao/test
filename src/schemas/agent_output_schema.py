...
StepType = Literal["thought", "hypothesis", "action", "finish"]
...
class AgentStep:
    ...
    def __post_init__(self) -> None:
        if self.step_type in {"thought"} and not self.reasoning:
            raise ValueError("reasoning is required for thought steps")

        if self.step_type == "hypothesis":
            if self.hypothesis is None or not self.hypothesis.strip():
                raise ValueError("hypothesis must be provided when step_type='hypothesis'")
